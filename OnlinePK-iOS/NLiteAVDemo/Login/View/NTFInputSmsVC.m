...
{
    // 进入后台停止计时器
    if (self.countDownBtn.counting) { return; }
...
- (void)setupViews
{
    ...
    self.infoLab.frame = CGRectMake(self.titleLab.left, self.titleLab.bottom + 8, self.titleLab.width, 44);
    ...
    self.nextBtn.frame = CGRectMake(self.titleLab.left, self.countDownBtn.bottom + 50, self.titleLab.width, 50);
    self.protocolView.frame = CGRectMake(self.titleLab.left, self.nextBtn.bottom + 30, self.titleLab.width, 30);
...
- (void)nextAction
{
    if ([_mobile length] > 0 && [_smsCode length] > 0) {
        ...
        [NEAccount loginWithMobile:_mobile smsCode:_smsCode completion:^(NSDictionary * _Nullable data, NSError * _Nullable error) {
...
- (NSAttributedString *)countDownTextWithSecond:(int32_t)second
{
    ...
    NSMutableAttributedString *attr = [[NSMutableAttributedString alloc] initWithString:secondStr attributes:@{NSForegroundColorAttributeName: HEXCOLOR(0x2953FF)}];
    NSAttributedString *tmp = [[NSAttributedString alloc] initWithString:@" 后重新发送验证码" attributes:@{NSForegroundColorAttributeName: HEXCOLOR(0x333333)}];
...
{
    if (inputView != self.inputView) { return; }
    _smsCode = content;
    BOOL enable = NO;
...
- (UIButton *)nextBtn
{
    if (!_nextBtn) {
        ...
        [_nextBtn setBackgroundImage:[UIImage ne_imageWithColor:disableCol] forState:UIControlStateDisabled];
