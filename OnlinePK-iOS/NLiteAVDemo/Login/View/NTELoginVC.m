...
- (void)setupSubviews
{
    ...
    self.phoneNumField.frame = CGRectMake(self.verLine.right + 10, self.areaLab.top, self.titleLab.width - self.verLine.right - 10 , 44);
    self.horLine.frame = CGRectMake(self.titleLab.left, self.phoneNumField.bottom + 1, self.titleLab.width, 1);
    self.tipLab.frame = CGRectMake(self.titleLab.left, self.horLine.bottom + 10, self.titleLab.width, 18);
    self.getSmsBtn.frame = CGRectMake(self.titleLab.left, self.tipLab.bottom + 36, self.titleLab.width, 50);
...
{
    NSString *validRegEx = @"^1[356789]\\d{9}$";
    NSPredicate *reg = [NSPredicate predicateWithFormat:@"SELF MATCHES %@", validRegEx];
...
- (UIButton *)getSmsBtn
{
    if (!_getSmsBtn) {
        ...
        [_getSmsBtn setBackgroundImage:[UIImage ne_imageWithColor:disableCol] forState:UIControlStateDisabled];
        _getSmsBtn.layer.cornerRadius = 25;
        _getSmsBtn.layer.masksToBounds = YES;
        [_getSmsBtn addTarget:self action:@selector(getSmsAction) forControlEvents:UIControlEventTouchUpInside];
    }
    return _getSmsBtn;
}
