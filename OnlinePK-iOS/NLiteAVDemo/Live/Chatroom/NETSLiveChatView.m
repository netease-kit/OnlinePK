...
@interface NETSLiveChatView () <UITableViewDelegate,UITableViewDataSource,UIGestureRecognizerDelegate>
{
    CGRect _preRect;
}
@property (nonatomic,strong) NSMutableArray<NETSMessageModel *> *messages;
///缓存的插入消息,聊天室需要在另外个线程计算高度,减少UI刷新
@property (nonatomic,strong) NSMutableArray *pendingMessages;
@end

@implementation NETSLiveChatView
...
{
    return [NETSLiveChatCell heightWithIndexPath:indexPath datas:self.messages];
}

- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    return [NETSLiveChatCell cellWithTableView:tableView indexPath:indexPath datas:self.messages];
}
...
- (UITableView *)tableView
{
    if (!_tableView) {
        ...
        [_tableView registerClass:[NETSLiveChatCell class] forCellReuseIdentifier:[NETSLiveChatCell description]];
...
- (void)processPendingMessages
{
    ...
    for (NIMMessage *message in insert)
    {
        ...
        if ([message.remoteExt[@"type"] integerValue] == 1) {
            model.type = NETSMessageNotication;
        }
...
- (void)addModels:(NSArray<NETSMessageModel *> *)models
{
...
- (void)scrollToBottom:(NSArray<NETSMessageModel *> *)newModels
{
    UIEdgeInsets insets = self.tableView.contentInset;
    if (insets.top != 0 ) {
        CGFloat height = 0;
        for (NETSMessageModel *model in newModels) {
