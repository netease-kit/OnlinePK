...
- (void)_sendPkEndImMsgWithData:(nullable NETSPassThroughHandlePunishData *)data
                       pkResult:(NETSPkResult)pkResult
                       errorPtr:(NSError * __nullable *)errorPtr
{
    NSString *roomId = self.singleRoom.chatRoomId;
    
    int32_t currentAnchorWin = [self _currentAnchorWinWithResult:pkResult];
...
- (int32_t)_currentAnchorWinWithResult:(NETSPkResult)result
{
    if (result == NETSPkCurrentAnchorWin) {
        return 1;
    } else if (result == NETSPkOtherAnchorWin) {
        return -1;
    }
    return 0;
}
