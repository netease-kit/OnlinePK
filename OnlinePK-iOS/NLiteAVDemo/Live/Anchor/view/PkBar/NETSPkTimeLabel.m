...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        _prefix = @"PK ";
        self.text = [NSString stringWithFormat:@"%@00:00", _prefix];
    ...
    dispatch_source_set_event_handler(_timer, ^{
        if (timeout <= 0) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                self.text = [NSString stringWithFormat:@"%@00:00", self.prefix];
            });
        } else {
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                self.text = [NSString stringWithFormat:@"%@%d:%@", self.prefix, minutes, secondStr];
                timeout--;
            });
        }
    });
