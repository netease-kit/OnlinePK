...
public abstract class Program {
  ...
  public Program(Context context, int vertexShaderResourceId, int fragmentShaderResourceId) {
    this(
        Extensions.readTextFileFromResource(context, vertexShaderResourceId),
        Extensions.readTextFileFromResource(context, fragmentShaderResourceId));
  }
  ...
  public void drawFrame(
      int textureId, float[] texMatrix, float[] mvpMatrix, int x, int y, int width, int height) {
    GLES20.glGetIntegerv(GLES20.GL_VIEWPORT, mOriginViewport, 0);
    GLES20.glViewport(x, y, width, height);
    drawFrame(textureId, texMatrix, mvpMatrix);
    GLES20.glViewport(
        mOriginViewport[0], mOriginViewport[1], mOriginViewport[2], mOriginViewport[3]);
  }

  /**
   * Releases the program.
   *
   * <p>The appropriate EGL context must be current (i.e. the one that was used to create the
   * program).
   */
  public void release() {
