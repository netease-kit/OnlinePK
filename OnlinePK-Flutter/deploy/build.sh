...
#sh "${PROJECT_PATH}/deploy/pack_env.sh" "$ENV"
...
ARCHIVE_DIRECTOR_NAME="flutter_pklive_${now}_${BUILD_COMMIT_ID}_${BUILD_EVN}"
ARCHIVE_NAME="flutter_pklive_${now}_${BUILD_COMMIT_ID}_${BUILD_EVN}"
...
if [ "$BACKUP_DIR" != "" ] && [ "$BACKUP_DIR" != "--" ]; then
    ...
    sh $PROJECT_PATH/build_tools/backup/upload-artifacts.sh outputs/. "livekit/$BACKUP_DIR"
    ...
    sh $PROJECT_PATH/build_tools/notification/notify.sh --platform "$PLATFORM" --env "${ENV}" --version "${VERSION_NAME}" \
      --downloadurl "http://10.242.100.195/livekit/$BACKUP_DIR/v$directory_name/$platform_lowercase"
    rm -rf build_tools
fi
## upload done
...
directory_name="${VERSION_NAME}"
platform_lowercase=$(echo "$PLATFORM" | tr '[A-Z]' '[a-z]')
download_url=http://10.242.100.195/livekit/$BACKUP_DIR/v$directory_name/$platform_lowercase/$ARCHIVE_NAME
