...
import 'package:livekit_pk/service/client/app_http_client.dart';
import 'package:livekit_pk/service/client/http_code.dart';
import 'package:livekit_pk/service/proto/base_proto.dart';
...
abstract class AppHttpProto<T> extends BaseProto {
  ...
  Future<Result<T>> execute() async {
    ...
    if (response == null) {
      LiveLog.e(_tag, 'resp path=$url result is null');
      return Result(code: HttpCode.netWorkError);
    } else {
      if (response.statusCode == HttpCode.success) {
        LiveLog.d(_tag, 'resp path=$url code = 200 , result ${response.data}');
        ...
        LiveLog.e(_tag, 'resp path=$path code${response.statusCode}');
