...
class _NELiveMediaControllerImpl extends NELiveMediaController
  ...
  Future<VoidResult> disableLocalAudio() {
    ...
    if (context != null) {
      return context.rtcController.pauseLocalAudioRecording();
    } else {
  ...
  Future<VoidResult> enableLocalAudio() {
    ...
    var context = NELiveKit.instance._anchorLiveInfo.liveRoom;
    if (context != null) {
      return context.rtcController.resumeLocalAudioRecording();
    } else {
      return Future.value(const NEResult(code: -1, msg: 'not in room'));
    }
  }
  ...
  Future<VoidResult> startLastmileProbeTest(
      NERoomRtcLastmileProbeConfig config) async {
    commonLogger.i('startLastmileProbeTest');
    var ret = await NERoomKit.instance.roomService
        .previewRoom(NEPreviewRoomParams(), NEPreviewRoomOptions());
    return ret.nonNullData.previewController.startLastmileProbeTest(config);
  }

  @override
  Future<VoidResult> stopLastmileProbeTest() async {
    commonLogger.i('stopLastmileProbeTest');
    var ret = await NERoomKit.instance.roomService
        .previewRoom(NEPreviewRoomParams(), NEPreviewRoomOptions());
    return ret.nonNullData.previewController.stopLastmileProbeTest();
  }
}
