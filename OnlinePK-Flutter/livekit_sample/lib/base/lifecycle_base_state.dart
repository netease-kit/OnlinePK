...
abstract class LifecycleBaseState<T extends StatefulWidget>
  ...
  Future<T?> lifecycleExecute(Future<T> future) {
    prepareExecutor();
    return _executor!.exec(future);
  }

  Future<T?> lifecycleExecuteUI(Future<T> future) {
    prepareExecutor();
    return _executor!.execUi(future);
  }

  UniqueKey? lifecycleListen(Stream<T> stream, void Function(T event) onData) {
    prepareExecutor();
  ...
  void delayTask(void Function() callback,
      {int days = 0,
      int hours = 0,
      int minutes = 0,
      int seconds = 0,
